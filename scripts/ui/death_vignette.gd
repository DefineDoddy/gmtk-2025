...
func _on_death_wait_timeout():
	...
	tween.finished.connect(_on_fade_out_finished, CONNECT_ONE_SHOT)
...
func fade_out_vignette():
	...
	tween.tween_method(
		func(val): material.set_shader_parameter("alpha", val),
		material.get_shader_parameter("alpha"), 0.0, 1.0
	)
	tween.tween_method(
		func(val): material.set_shader_parameter("inner_radius", val),
		material.get_shader_parameter("inner_radius"), 0.55, 1.0
	)
	tween.tween_method(
		func(val): material.set_shader_parameter("outer_radius", val),
		material.get_shader_parameter("outer_radius"), 1.235, 1.0
	)
	tween.tween_method(
		func(val): material.set_shader_parameter("color", val),
		material.get_shader_parameter("color"), Color(0.282, 0, 0), 1.0
	)
